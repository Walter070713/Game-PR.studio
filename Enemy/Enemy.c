...
// Initialize enemy pool with default runtime values.
void InitEnemy(Enemy enemypool[], int emycapacity)
{
    if (!enemypool || emycapacity <= 0) return;
...
// Update enemy movement/chase and damage flash lifetime.
...
// Draw active enemies and optional labels.
